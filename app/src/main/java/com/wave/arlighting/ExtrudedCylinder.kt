...
object ExtrudedCylinder {
  ...
  fun makeExtrudedCylinder(
    ...
    val submesh = Submesh.builder().setTriangleIndices(triangleIndices)
      .setMaterial(material).build()

    return RenderableDefinition.builder().setVertices(vertices)
      .setSubmeshes(listOf(submesh)).build()
  }
  ...
  private fun generateVerticesFromPoints(
    ...
    directionFromTopToBottom = Quaternion.rotateVector(
      rotationFromAToB,
      Vector3.forward()
    ).normalized()
    val rightDirection = Quaternion.rotateVector(
      rotationFromAToB,
      Vector3.right()
    ).normalized()
    val upDirection = Quaternion.rotateVector(
      rotationFromAToB,
      Vector3.up()
    ).normalized()
    ...
    for (edgeIndex in 0..NUMBER_OF_SIDES) {
      ...
      var normal = Vector3.subtract(
        topPosition,
        directionFromTopToBottom.scaled(-halfHeight)
      ).normalized()
      topPosition = Vector3.add(
        topPosition,
        center
      )
      var uvCoordinate = UvCoordinate(
        uStep * edgeIndex,
        0f
      )
      var vertex = Vertex.builder().setPosition(topPosition).setNormal(normal)
      ...
      normal = Vector3.subtract(
        bottomPosition,
        directionFromTopToBottom.scaled(halfHeight)
      ).normalized()
      bottomPosition = Vector3.add(
        bottomPosition,
        center
      )
      ...
      vertex = Vertex.builder().setPosition(bottomPosition).setNormal(normal)
        .setUvCoordinate(uvCoordinate).build()
  ...
  private fun updateConnectingPoints(
    ...
    for (segmentIndex in 0 until points.size - 2) {
      ...
      for (edgeIndex in 0..NUMBER_OF_SIDES) {
        ...
        val previousSegmentVertexIndex =
          currentSegmentVertexIndex - NUMBER_OF_SIDES - 1
        val updatedVertex =
          Vertex.builder().setPosition(position).setNormal(normal)
            .setUvCoordinate(
              UvCoordinate(
  ...
  private fun generateTriangleIndices(
  ...
  private fun makeDisk(
    ...
    val nextPoint =
      points[centerPointIndex + (if (direction == Direction.UP) 1 else -1)]
    val normal = Vector3.subtract(
      centerPoint,
      nextPoint
    ).normalized()
    val center = Vertex.builder().setPosition(centerPoint).setNormal(normal)
      .setUvCoordinate(
        UvCoordinate(
          .5f,
          .5f
    ...
    for (edge in 0..NUMBER_OF_SIDES) {
      ...
      val uvCoordinate = UvCoordinate(
        (cos(theta.toDouble()) + 1f).toFloat() / 2,
        (sin(theta.toDouble()) + 1f).toFloat() / 2
      )
      val topVertex =
        Vertex.builder().setPosition(edgeVertex.position).setNormal(normal)
          .setUvCoordinate(uvCoordinate).build()
