...
class LineSimplifier {
  ...
  fun append(point: Vector3) {
    if (points.isNotEmpty() && Vector3.subtract(
        points.last(),
        point
      ).length() < MINIMUM_DISTANCE_BETWEEN_POINTS
    ) {
      return
    }
    points.add(point)
  ...
  private fun smoothPoints() {
    val pointsToSmooth = points.subList(
      points.size - POINT_SMOOTHING_INTERVAL - 1,
      points.size - 1
    )
  ...
  private fun smoothPoints(pointsToSmooth: List<Vector3>): ArrayList<Vector3> {
    ...
    for (i in 0 until endIndex - 1) {
      distance = getPerpendicularDistance(
        points[0],
        points[endIndex],
        points[i]
      )
    ...
    if (maxDistance > MAXIMUM_SMOOTHING_DISTANCE) {
      val result1 = smoothPoints(
        pointsToSmooth.subList(
          0,
          index
        )
      )
      val result2 = smoothPoints(
        pointsToSmooth.subList(
          index + 1,
          endIndex
        )
  ...
  private fun getPerpendicularDistance(
    start: Vector3,
    end: Vector3,
    point: Vector3
  ): Float {
    val crossProduct = Vector3.cross(
      Vector3.subtract(
        point,
        start
      ),
    ...
    return crossProduct.length() / Vector3.subtract(
      end,
      start
    ).length()
  }
