...
class Drawing(
  ...
  companion object {
    ...
    private fun plane(hitResult: HitResult?): CollisionPlane? {
      return (hitResult?.trackable as? Plane)?.let {
        ...
        val normal = Quaternion.rotateVector(
          pose.rotation(),
          Vector3.up()
        )
        CollisionPlane(
  ...
  private fun render() {
    val definition = ExtrudedCylinder.makeExtrudedCylinder(
      RADIUS,
      line.points,
      material ?: return
    ) ?: return
    if (renderable == null) {
      ModelRenderable.builder().setSource(definition).build()
        .thenAccept { renderable = it }
    } else {
  ...
  fun extend(
    ...
    val ray = scene?.camera?.screenPointToRay(
      x,
      y
    ) ?: return
    if (plane != null) {
      val rayHit = RayHit()
      if (plane.rayIntersection(
          ray,
          rayHit
        )
      ) {
...
class Link(
  ...
  companion object {
    fun warmup(
      ...
      }.setRegistryId(uri.toString()).build().exceptionally {
        Toast.makeText(
          context,
          it.message,
          Toast.LENGTH_LONG
...
class CloudAnchor(
  ...
  init {
    ...
    ViewRenderable.builder().setView(
      context.applicationContext,
      R.layout.view_renderable_cloud_anchor
    ).build().thenAccept { renderable = it }
  }
  ...
  fun copyToClipboard(context: Context) {
    val clip = ClipData.newPlainText(
      context.getString(R.string.cloud_anchor_id_label),
      id() ?: return
    )
...
class Video(
  ...
  init {
    ModelRenderable.builder().setSource(
      ...
      it.material.setExternalTexture(
        "videoTexture",
        texture
      )
      it.material.setFloat4(
        "keyColor",
        Color(
          0.1843f,
          1.0f,
